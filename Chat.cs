...
public class Chat : Hub
{
    ...
    public override Task OnConnected()
    {
        ...
        LogOnModel user = OnlineUser.userObj.Where(item => item.sessionId == HttpContext.Current.Request.Cookies["ASP.NET_SessionId"].Value.ToString()).SingleOrDefault();
        user.connectionId = Context.ConnectionId;
        return Clients.All.joined(Context.ConnectionId, newUsers);
    }

    public void Send(string message, string groupName, int userid,int FriendId)
    {
        if (Clients != null)
        {
            Clients.Group(groupName).addMessage(message, groupName, userid, FriendId);
        }
    }
    ...
    public void CreateGroup(string currentUserId, string toConnectTo)
    {
        ...
        bool isOnline = false;
        var newUsers = OnlineUser.userObj.Where(item => item.newStatus == true && item.userId == toConnectTo).Select(item => item.userId).FirstOrDefault();
        if (newUsers != null)
            isOnline = true;
